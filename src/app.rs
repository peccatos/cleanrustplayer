// Interactive shell for local playback and admin commands.
...
impl App {
    ...
    fn print_provider_search_results(&self, query: &str) {
        let report = self
            .context
            .search_service
            .search(SearchQuery::new(query, 10));
    ...
    fn print_resolved_media(&self, url: &str) {
        ...
        match self.context.search_service.resolve(normalized) {
            Ok(media) => {
                println!("Resolved:");
    ...
    fn open_source(&mut self, source: &str) -> Result<()> {
        ...
        if normalized.starts_with("http://") || normalized.starts_with("https://") {
            println!("HTTP links are handled in the desktop frontend");
            return Ok(());
        }
